...
func newBackendSlot(ctx context.Context, backend upstream.Backend) *backendSlot {
	rt := backend.Cfg().Rate
	if rt < 1 {
		rt = 1
	}
...
func (c *BackendCluster) Fetch(rule *config.Rule, ctx *fasthttp.RequestCtx, r *fasthttp.Request) (
	...
	if len(*slots) == 0 {
		return nil, nil, upstream.defaultReleaser, ErrNoBackends
	}
	...
	return nil, nil, upstream.defaultReleaser, ErrAllBackendsAreBusy
}
...
func (c *BackendCluster) Add(cfg *config.Backend) error {
	...
	backend := upstream.NewBackend(c.ctx, cfg, id)
...
func (c *BackendCluster) Update(ctx context.Context, id string, cfg *config.Backend) error {
	...
	var be upstream.Backend
	if cfg != nil {
		be = upstream.NewBackend(ctx, cfg, id)
	} else {
		be = slot.backend
	}
