...
func (s *HTTP) initServer(r *router.Router, middlewares []middleware.HttpMiddleware) {
	s.server = &fasthttp.Server{
		...
		Concurrency:                   fasthttp.DefaultConcurrency, // Configure the maximum number of concurrently handled requests to support extreme load.
		ReadBufferSize:                32784,                       // Allocate a 32 KiB read buffer, aligned with typical CPU cache lines for optimal throughput.
		WriteBufferSize:               32784,                       // Allocate a 32 KiB read buffer, aligned with typical CPU cache lines for optimal throughput.
		...
		MaxRequestBodySize:            0,                           // Due to GetOnly mode.
	}
}
